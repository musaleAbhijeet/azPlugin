...
public class MyCordovaPlugin extends CordovaPlugin {
    ...
    private static final String COULD_NOT_READ_REDIRECT_URI_SCHEME_MSG = "Could not read redirect uri scheme from manifest. "
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        if ("loginWithGoogle".equals(action)) {
            ...
            return getTable(args, callbackContext);
        }

        else if ("insertInToTable".equals(action)) {
            return insertInToTable(args, callbackContext);
        }

        else if ("deleteData".equals(action)) {
            return deleteData(args, callbackContext);
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            resultContext.sendPluginResult(new PluginResult(Status.ERROR,
                    "Cannot authenticate due to error occured. " + data.getDataString()));
        }
    }
    ...
    private String getUriSchemeFromManifest() throws Exception {
        ...
        ApplicationInfo ai;
        try {
            ai = activity.getPackageManager().getApplicationInfo(activity.getPackageName(),
        ...
        if (uriScheme == null || uriScheme.isEmpty()) {
            String message = "'.azure_mobileapps_redirect_uri_scheme' meta-data value is empty in your AndroidManifest.xml. "
    ...
    /**
     * Convert MobileServiceUser object to javascript 'Token' object to pass it in
     * callback
     *
    ...
    private boolean login(CordovaArgs args, CallbackContext callbackContext) {
        final String appUrl = args.getString(0);
        final String loginType = args.getString(1);
        mobileServiceClient = new MobileServiceClient(appUrl, this.cordova.getActivity());
        data = mobileServiceClient.login(loginType);
        if (data.userId != null) {

        }
    }

    private boolean getTable(CordovaArgs args, CallbackContext callbackContext) {
        final String tableName = args.getString(0);
        table1 = mobileServiceClient.getTable(tableName);
        result = table1.read();
    }

    private boolean insertInToTable(CordovaArgs args, CallbackContext callbackContext) {
        final String idToinsert = args.getString(0);
        final String value = args.getString(1);
        final String tableName = args.getString(2);
        table1 = mobileServiceClient.getTable(tableName);
        // newItem = {
        // id: idToinsert,
        // name: name
        // };
        table1.insert(newItem);
    }

    private boolean deleteData(CordovaArgs args, CallbackContext callbackContext) {
        final String idToDelete = args.getString(0);
        final String tableName = args.getString(1);
        table1 = mobileServiceClient.getTable(tableName);
        // table1.del({'id':idToDelete});
    }
}